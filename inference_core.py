...
class InferenceCore:
    ...
    def interact(self, mask, idx, total_cb=None, step_cb=None):
        ...
        # print("mask_device:",mask[1:].device)
        # print("image_buffer_device:",self.get_image_buffered(idx).device)
        ...
        for ti in range(self.t):    # <-- Pick up here, use self.prob1 as soft mask for the RiTM inizialization of prev mask
    ...
    def interact_can_output_soft_masks_vujas(self, mask, idx, total_cb=None, step_cb=None):
        """
        Interact -> Propagate -> Fuse
        ...
        out_masks_soft = self.prob2

        # Trim paddings
        if self.pad[2] + self.pad[3] > 0:
            out_masks_soft = out_masks_soft[:, :, :, self.pad[2]:-self.pad[3], :]
        if self.pad[0] + self.pad[1] > 0:
            out_masks_soft = out_masks_soft[:, :, :, :, self.pad[0]:-self.pad[1]]

        self.out_masks_soft = out_masks_soft.detach().cpu().numpy()

        return self.np_masks, self.out_masks_soft
