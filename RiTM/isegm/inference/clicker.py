...
class Clicker(object):
    ...
    def make_next_click(self, pred_mask):
        assert self.gt_mask is not None             # Verify that a GT mask was given at the initialization
        click = self._get_next_click(pred_mask)     # Give current predicted mask so that a click can be inferred with the GT mask
        self.add_click(click)                       # Add the click to the click memory
    ...
    def _get_next_click(self, pred_mask, padding=True):
        ...
        return Click(is_positive=is_positive, coords=(coords_y[0], coords_x[0]))    # Create a Click
