...
from RiTM.isegm.data.base import ISDataset
from RiTM.isegm.data.sample import DSample
import os
...
class CocoLvisDataset(ISDataset):
    ...
    def get_sample(self, index) -> DSample:
        ...
        new_image_path_list = str(image_path).split('/')[:-2]
        new_image_path_list.append(str(image_path).split('/')[-1])
        new_image_path = os.path.join(*new_image_path_list) # using the splat operator

        image = cv2.imread(str(new_image_path))
