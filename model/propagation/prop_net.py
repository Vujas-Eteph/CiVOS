...
class EvalMemoryReader(nn.Module):
    ...
    def forward(self, mk, mv, qk):
        ...
        # Got this error... So adapting the code
...
class PropagationNetwork(nn.Module):
    ...
    def memorize(self, frame, masks): 
        ...
        if frame.device not in [masks.device, others.device] and masks.device is not others.device:
            print("frame.device:",frame.device)
            print("masks.device:",masks.device)
            print("others.device:",others.device)
